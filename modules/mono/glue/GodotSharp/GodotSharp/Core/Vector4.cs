...
namespace Godot
{
    ...
    public struct Vector4 : IEquatable<Vector4>, IComparable<Vector4>
    {
        ...
        public readonly real_t ManhattanDistanceTo(Vector4 to)
        {
            return Mathf.Abs(X - to.X) + Mathf.Abs(Y - to.Y) + Mathf.Abs(Z - to.Z) + Mathf.Abs(W - to.W);
        }
        ...
        public readonly int CompareTo(Vector4 other)
        {
        ...
        public readonly bool IsExactlyZero()
        {
            ...
            ulong w = BitConverter.DoubleToUInt64Bits(W);
            return ((x | y | z | w) & 0x7FFFFFFFFFFFFFFFUL) == 0u;
#else
            uint x = BitConverter.SingleToUInt32Bits(X);
            uint y = BitConverter.SingleToUInt32Bits(Y);
            uint z = BitConverter.SingleToUInt32Bits(Z);
            uint w = BitConverter.SingleToUInt32Bits(W);
            return ((x | y | z | w) & 0x7FFFFFFFu) == 0u;
#endif
        }
